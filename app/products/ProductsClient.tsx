...
import { ProductExpandPanel } from "./ProductExpandPanel"; // Panelini içe aktardık
...
export default function ProductsClient({ products, categories }: { products: Product[]; categories: Category[]; }) {
  ...
  const openId = sp?.get("open"); // Otomatik açılacak ürün ID'si
  ...
  // URL Parametreleri ile Modalı Tetikleme
  useEffect(() => {
    if (openId && products) {
  ...
  const filteredAll = useMemo(() => {
    ...
    return [...result].sort((a, b) => {
      ...
      return sortOption === "price_asc" ? priceA - priceB : sortOption === "price_desc" ? priceB - priceA : dateB - dateA;
    });
    ...
    <div className="container mx-auto px-4 mt-4 max-w-[1400px]">
      <div className="flex flex-col lg:flex-row relative gap-4 lg:gap-8">
        ...
        <ProductFilter 
          ...
          onClearAll={() => { setSelectedCategories([]); updateUrl({ cats: null }); }}
        />
        ...
        <div className="w-full lg:w-3/4">
          ...
          />
          <CatalogPagination page={page} pageCount={pageCount} onChange={(p) => { setPage(p); updateUrl({ page: String(p) }); }} />
        </div>
      </div>

      {/* DETAY PANELİ (Global Modal) */}
      {selectedProduct && (
